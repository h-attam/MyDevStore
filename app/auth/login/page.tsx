...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (email === 'admin@example.com' && password === 'admin123') {
        // Admin için özel giriş (Supabase'de admin kullanıcısı olmalı)
        ...
        if (error) {
          ...
          if (signUpData.user) {
            alert('Admin hesabı oluşturuldu. Lütfen tekrar giriş yapın.')
            return
          }
        } else if (data.user) {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      setErrors({ general: error.message || 'Google ile giriş yapılırken bir hata oluştu' })
    }
  }
